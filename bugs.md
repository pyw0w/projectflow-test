...
в тесте валит тест**. Итог: **16 файлов, 70/70 passed** (на момент бага — 11/40).
...
| 41 | `LoadMore`: счётчик `N of M` + кнопка Show more | `LoadMore.test.jsx` |
| 42 | `LoadMore`: клик вызывает `onNext` | там же |
| 43 | `LoadMore`: всё показано → ничего не рендерится | там же |
| 44 | `useProjects`: пустое хранилище → [] | `useProjects.test.js` |
| 45 | `useProjects`: addProject нормализует (trim), задаёт id/createdAt/isPrivate | там же |
| 46 | `useProjects`: новый проект первым в списке | там же |
| 47 | `useProjects`: персист + переживает remount (симуляция reload) | там же |
| 48 | `useProjects`: removeProject по id + персист удаления | там же |
| 49 | `useProjects`: битый JSON в хранилище → [] | там же |
| 50 | `useProjects`: отбраковка битых записей без id/name | там же |
| 51 | `ProjectForm`: закрыта → нечего не рендерится | `ProjectForm.test.jsx` |
| 52 | `ProjectForm`: диалог `aria-modal`, все поля с лейблами | там же |
| 53 | `ProjectForm`: пустое имя блокируется + alert-ошибка | там же |
| 54 | `ProjectForm`: submit отправляет trim-значения + isPrivate | там же |
| 55 | `ProjectForm`: закрытие по Cancel/крестику/Escape/backdrop | там же |
| 56 | `ProjectForm`: повторное открытие сбрасывает значения | там же |
| 57 | `ProjectCard`: имя — внешняя ссылка при наличии URL | `ProjectCard.test.jsx` |
| 58 | `ProjectCard`: без URL — текст, не ссылка | там же |
| 59 | `ProjectCard`: бейдж Private только для приватных | там же |
| 60 | `ProjectCard`: языковой точка с GitHub-цветом | там же |
| 61 | `ProjectCard`: описание или заглушка «No description» | там же |
| 62 | `ProjectCard`: кнопка удаления отдаёт id | там же |
| 63 | `ProjectCard`: дата добавления отображается | там же |
| 64 | `ProjectsPage`: пустое состояние на чистом браузере | `ProjectsPage.test.jsx` |
| 65 | `ProjectsPage`: рендер из сохранённого хранилища (reload) | там же |
| 66 | `ProjectsPage`: создание через модалку → карточка + запись в localStorage | там же |
| 67 | `ProjectsPage`: live-фильтр + пустое состояние при промахе | там же |
| 68 | `ProjectsPage`: фильтр матчит описание и язык | там же |
| 69 | `ProjectsPage`: удаление из списка и из хранилища | там же |
| 70 | `ProjectsPage`: тулбар использует общий компонент (контракт 420px) | там же |
...
### 2.4 Пагинация UI — `scripts/pagination-check.mjs` (headless Chromium)

Выполняется при запущенном dev-сервере: `node scripts/pagination-check.mjs`.
**8/8 PASS** (на живом аккаунте: 39 репозиториев):

- [x] первая страница ограничена 20 карточками
- [x] счётчик `20 of N repositories`
- [x] кнопка «Show more» видна при скрытых карточках
- [x] клик догружает следующие 20 (или все оставшиеся)
- [x] в конце все репозитории показаны, кнопка исчезает
- [x] фильтр сбрасывает пагинацию на первую страницу (20 из 31 после ввода)

### 2.5 Страница «Проекты» — `scripts/projects-check.mjs` (headless Chromium)

Выполняется при запущенном dev-сервере: `node scripts/projects-check.mjs`.
**19/19 PASS**:

- [x] маршрутизация: `#/` → Repo Search, `#/projects` → Projects (без перезагрузки)
- [x] пустое состояние на чистом браузере
- [x] десктоп: поиск и кнопка не перекрываются, тулбар row
- [x] валидация: пустое имя блокируется, диалог остаётся открытым
- [x] создание проекта через форму ( Private → карточка + бейдж + счётчик 1)
- [x] переживает reload (запись в localStorage на месте)
- [x] live-поиск: совпадение, промах → empty state
- [x] Escape закрывает диалог
- [x] удаление + персист удаления после reload
- [x] 375px: нет перекрытия, тулбар column, всё внутри вьюпорта

Регрессия после переноса страницы в `src/pages/` (поиск не изменился):
manual-check **20/20**, pagination-check **8/8**, `npm test` **70/70**.

### 2.6 Сценарии, проверенные вручную в ходе разработки
